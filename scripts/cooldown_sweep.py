#!/usr/bin/env python3
"""
EMA Cooldown Sweep — Backtest cooldown durations after stop-loss.
Uses 4H candles from Hyperliquid (matching live signal engine) to test
cooldown periods of 0, 4h, 8h, 12h, 24h, 48h after stop-loss events.

Usage:
    python scripts/cooldown_sweep.py
    python scripts/cooldown_sweep.py --days 365
    python scripts/cooldown_sweep.py --asset hyperliquid
"""
...
def fetch_4h_ohlc(coingecko_id: str, days: int = 180) -> pd.DataFrame:
    """
...
def run_sweep(asset_id: str, days: int = 180):
    """Run all V7 cooldown configs against one asset using 4H candles."""
    print(f"\n{'='*70}")
    print(f"  COOLDOWN SWEEP (4H): {asset_id.upper()} ({days} days)")
    print(f"{'='*70}\n")
    ...
    for config_name in configs:
        ...
        cooldown_bars = config.get("ema_cooldown_bars", 0)
        cooldown_hours = cooldown_bars * 4
        ...
        for t in ema_trades:
            ...
            print(f"    {marker} {direction:5s} {entry} -> {exit_d}  {pnl:+.2f}% (${pnl_usd:+.2f})  [{reason}]")
...
def print_comparison(asset_id: str, results: list[dict]):
    ...
    header = (
        f"{'Cooldown':>10s} | {'Trades':>6s} | {'W':>3s} | {'L':>3s} | "
    ...
    for r in results:
        ...
        delta_str = f"${delta:+.2f}" if r["cooldown_h"] > 0 else "baseline"
        print(
            f"{r['cooldown_h']:>7d}h   | {r['trades']:>6d} | {r['wins']:>3d} | {r['losses']:>3d} | "
...
def main():
    parser = argparse.ArgumentParser(description="EMA Cooldown Sweep (4H candles)")
    ...
    if len(all_results) > 1:
        ...
        for i, ch in enumerate(cooldown_hours):
            ...
            if i > 0 and delta > best_delta:
                best_delta = delta
                best_idx = i
            delta_str = f" ({delta:+.0f})" if ch > 0 else ""

            print(
                f"  {ch:>3d}h: {total_trades:>3d} trades ({total_wins}W/{total_losses}L, "
                f"{win_rate}%), ${total_pnl:>9.2f} P&L{delta_str}, "
                f"{total_sl} stop-losses, {total_avoided} avoided"
            )

        best_ch = cooldown_hours[best_idx]
        if best_delta > 0:
            print(f"\n  RECOMMENDATION: {best_ch}h cooldown (+${best_delta:.2f} vs baseline)")
        elif best_delta == 0:
            print(f"\n  RESULT: No cooldown impact in this period (no stop-loss exits to trigger cooldowns)")
        else:
            print(f"\n  RESULT: All cooldowns reduce P&L — cooldown not recommended for this period")
