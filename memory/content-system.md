...
| **Model** | Claude Sonnet |
| **Output** | 3 market-moving items (Sonnet) + unified narrative (Haiku rewrite) |
| **Delimiter** | `\|\|\|` between items (NOT period-based splitting) |
| **Distribution** | Social card uses `|||` items (X + Telegram), app + email use unified narrative |
| **Framing** | Forward-looking: "3 things moving markets today" |

**Dual-format storage (2026-03-26):** A single Sonnet call produces 3 `|||`-separated items (proven, stable). A second Haiku call rewrites those items into a unified narrative paragraph. `briefs.summary` = narrative (app + email), `briefs.context` = items (social). If Haiku fails, falls back to `|||` items for all surfaces.

**Narrative rules:** Open with 1 sentence capturing overall market mood. Weave items with natural transitions. Close with Vela as calm guide. Describe market fear honestly but always contextualize constructively ("these stretches have historically preceded recoveries"). Never leave the reader sitting in anxiety. No em dashes. Under 100 words.

**Data sources:** Latest signals from DB, CoinGecko batch prices, Fear & Greed index, BTC dominance, top 25 ranked news items.

**Lightweight by design:** ~3 API calls vs ~9+ for the full signal pipeline (plus 1 cheap Haiku call for narrative). Has recovery mechanism — if daily-digest fails, run-signals can generate a fallback digest.
...
### Education Posts (Social)

Generated by `content-generator.ts` as `education` type posts. These explain the nuance behind recent high-signal news events, making complex topics approachable.

**Source (2026-03-26):** Pulls highest-scored articles from last 48H via `rankAndBalanceNews()` (same pipeline as digests/breaking news). Previously used raw chronological last 12 items, which produced repetitive low-signal content.

**Cross-day dedup:** Before generating, checks `social_posts` from last 72H for entity/topic overlap to prevent re-posting the same story framed differently. The Tether auditor story was suggested 4+ times before this was added.
...
### Source Attribution (Social)
- Format: `[Source]` at the bottom of the tweet, not inline "via Source"
- Pick the single highest-signal source when multiple outlets cover the same story
- Article link goes in the reply tweet, not the main tweet

### Cite Tag Stripping (Defense-in-Depth)
Model outputs sometimes contain `<cite index="X-Y">` tags. These must be stripped at BOTH layers:
- **Backend:** `stripCiteTags()` in brief-generator, signal-persistence, events output
- **Frontend:** `stripCiteTags()` in helpers.ts before rendering any model-generated text

### Content Voice
- **Market fear ≠ reader anxiety.** Vela can describe what the market is feeling honestly, but must always contextualize it so readers feel informed, not anxious.
- Always project confidence and control regardless of market conditions.
- Reframe fear constructively: "historically preceded recoveries", "smart money is positioning during uncertainty".
- Vela presents as a guide in full control — never reactive or alarmed.
